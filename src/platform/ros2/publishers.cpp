...
void ResultPublisher::publishOdometry(const core::IMUST& xc)
{
  ...
  if (!isFinite(t_this.x()) || !isFinite(t_this.y()) || !isFinite(t_this.z()) || !isFinite(q_this.x()) ||
      !isFinite(q_this.y()) || !isFinite(q_this.z()) || !isFinite(q_this.w()))
  {
    RCLCPP_WARN(node->get_logger(),
                "Skipping TF publish for aft_mapped: non-finite pose (x=%.6f, y=%.6f, z=%.6f, q=[%.6f, %.6f, %.6f, "
                "%.6f])",
...
void ResultPublisher::publishCurrentPath(const pcl::PointCloud<core::PointType>& pcl_path)
{
  publishPointCloudTo(pcl_path, pub_curr_path);
}

void ResultPublisher::clearCurrentPath()
{
  pcl::PointCloud<core::PointType> empty_path;
  publishCurrentPath(empty_path);
}
...
void ResultPublisher::publishLocalTrajectory(PLV(3) & pwld, double jour, const core::IMUST& x_curr, int cur_session,
                                             pcl::PointCloud<core::PointType>& pcl_path)
{
  ...
  publishCurrentPath(pcl_path);
}

template <typename OctoTreePtr>
void ResultPublisher::publishLocalMap(int /*mgsize*/, std::unordered_map<core::VOXEL_LOC, OctoTreePtr>& surf_map,
...
template void ResultPublisher::publishLocalMap<mapping::OctoTree*>(
    int mgsize, std::unordered_map<core::VOXEL_LOC, mapping::OctoTree*>& surf_map, std::vector<core::IMUST>& x_buf);
